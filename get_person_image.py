...
def get_image_url(pax_name: str) -> str:
    ...
    with Session() as s:
        ...
        r = s.get(
            ...
            + pax_name
            + "&language=fr&origin=*&format=json"
        )
        if not r.ok:
            return
        ...
        if not r.ok:
            return
        image_url = r2.json()["entities"][q_id]["claims"]["P18"][0]["mainsnak"][
            "datavalue"
        ]["value"]

        r3 = requests.get(
            "https://commons.wikimedia.org/w/api.php?action=query&prop=imageinfo&iiprop=url&redirects&format=json&origin=*&titles=File:"
...
def save_image(img_url: str, out_file: str) -> None:
